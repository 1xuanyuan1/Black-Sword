...
- 不自行生成或替换用户确认的角色造型。宁霜华、玄灯与赵云均使用项目所有者提供的素材；赵云只接入无盾、无马的长枪步战版本。
...
赵云的 `actor_scene`、`portrait`、ID、数值和解锁条件均必须完整。项目使用后续交付并整理过的透明步战 Sprite Sheet，不引用旧带水印参考图；发布校验仍须检查 `CREDITS.md` 中的生成平台许可状态。
...
- 五张角色卡都必须有可加载头像与独立角色场景；赵云卡使用无盾、无马的长枪步战素材。发布构建在素材许可状态未确认时必须给出明确检查项。
...
- 五个道具和四条养成定义数值完整。
- 正式发布校验在任一角色缺少透明步战资源、头像或素材许可记录时必须失败；盾牌与骑马动画不属于赵云必需资源。
